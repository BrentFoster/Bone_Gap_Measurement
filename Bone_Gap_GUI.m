...
% handles    structure with handles and user data (see GUIDATA)
    ...
    switch which_button
        case 'space'
            ...
            [handles.x,handles.y, handles.gap_measurement, handles.angle_measurement] = Get_Measurements(handles, [], []);        
    end
...
function [x,y, gap_measurement, angle_measurement] = Get_Measurements(handles,x,y)
    ...
    % If there was only two click it is a bone gap measurement    
    if length(x) == 2
    ...
    elseif length(x) == 3
        ...
        % If there were three clicks it is a wrist angle measurement
        angle_measurement = Calculate_Angle(handles, x, y)       
        
        % Clear axes 3 (the intensity plot) since we aren't using it
        axes(handles.axes3)
        cla(handles.axes3)
        
    end

    % Set the GUI label to show this two measurements
    set(handles.gap_measurement_label, 'String', num2str(gap_measurement))   
    set(handles.angle_measurement_label, 'String', num2str(angle_measurement))

function angle_measurement = Calculate_Angle(handles, x, y)
    % Calculate the angle between the three given points    
    
    % Plot the angle measurement in a different color on axes 2
    axes(handles.axes2)
    hold on      
    plot(x, y, 'r*-')    
    ...
    angle_measurement = radtodeg(mod(atan2(x12*y32-x32*y12,x12*x32+y12*y32),2*pi)); 
     
    % Round the angle measurement to two decimal places
    angle_measurement = round(angle_measurement, 2);
...
%     gap_measurement = length(ndx_start:ndx_end) *  handles.images(handles.image_index).hdr.dime.pixdim(2);
    ...
    % Round the gap measurement to two decimal places
    gap_measurement = round(gap_measurement, 2);
...
function Show_Zoom_Image(handles, x, y)
    ...
    x_ndx = round(x(1) - handles.zoom_offset: x(end) + handles.zoom_offset);
    y_ndx = round(y(1) - handles.zoom_offset: y(end) + handles.zoom_offset);
...
function Show_Image(handles)
    ...
    cla(handles.axes2)
    cla(handles.axes3)
    set(handles.angle_measurement_label, 'String', num2str(0))    
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    handles.Output(handles.measure_num).angle_measurement = handles.angle_measurement;
